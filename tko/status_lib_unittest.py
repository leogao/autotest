...
class clean_raw_line_test(unittest.TestCase):
    ...
    def test_default(self):
        ...
        self.assertEqual(cleaned, raw_line_temp % '')
...
class line_buffer_test(unittest.TestCase):
    ...
    def test_get_single(self):
        buf = status_lib.line_buffer()
        buf.put("single line")
        self.assertEqual(buf.get(), "single line")
        self.assertRaises(IndexError, buf.get)
    ...
    def test_put_multiple_same_as_multiple_puts(self):
        ...
        counter = 0
        while buf_put.size():
            self.assertEqual(buf_put.size(), buf_multi.size())
            line = "line #%d" % counter
            self.assertEqual(buf_put.get(), line)
            self.assertEqual(buf_multi.get(), line)
            counter += 1
    ...
    def test_put_back_is_lifo(self):
        ...
        while buf.size():
            results.append(buf.get())
        self.assertEqual(results, ["1", "0", "1", "2", "3"])
...
class status_stack_test(unittest.TestCase):
    ...
    def test_default_to_nostatus(self):
        ...
        self.assertEqual(stack.current_status(), "NOSTATUS")
    ...
    def test_anything_overrides_nostatus(self):
        for status in self.statuses:
            stack = status_lib.status_stack()
            stack.update(status)
            self.assertEqual(stack.current_status(), status)
    ...
    def test_worse_overrides_better(self):
        for i in range(len(self.statuses)):
            ...
            for j in range(i + 1, len(self.statuses)):
                ...
                self.assertEqual(stack.current_status(),
                                  worse_status)
    ...
    def test_stack_is_lifo(self):
        ...
        stack.start()
        stack.update("WARN")
        self.assertEqual(stack.end(), "WARN")
        self.assertEqual(stack.end(), "FAIL")
        self.assertEqual(stack.end(), "GOOD")
        self.assertEqual(stack.end(), "NOSTATUS")
